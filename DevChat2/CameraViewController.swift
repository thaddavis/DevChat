...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.delegate?.shouldEnableCameraUI(enabled: false, line: "23")
        //cameraButton.isEnabled = false
        
        self.delegate?.shouldEnableRecordUI(enabled: false, line: "26")
        ...
        self.delegate?.shouldEnableCaptureModeControlUI(enabled: true)
    ...
    @IBOutlet internal weak var _previewView: PreviewView!
    ...
    private func configureSession() {
        ...
        /*
         We do not create an AVCaptureMovie FileOutput when setting up the session because the
         AVCaptureMovie FileOutput does not support movie recording with AVCaptureSessionPresetPhoto.
         */
    ...
    @IBAction internal func toggleCaptureMode(_ captureModeControl: UISegmentedControl) {
        if captureModeControl.selectedSegmentIndex == CaptureMode.photo.rawValue {
            
            self.delegate?.shouldEnableRecordUI(enabled: false, line: "324")
            //recordButton.isEnabled = false
            
            sessionQueue.async { [unowned self] in
                /*
                 Remove the AVCaptureMovie FileOutput from the session because movie recording is
                 not supported with AVCaptureSessionPresetPhoto. Additionally, Live Photo
                 capture is not supported when an AVCaptureMovie FileOutput is connected to the session.
                 */
        ...
        else if captureModeControl.selectedSegmentIndex == CaptureMode.movie.rawValue
        {
            ...
            sessionQueue.async { [unowned self] in
                ...
                if self.session.canAddOutput(movieFileOutput) {
                    ...
                    DispatchQueue.main.async { [unowned self] in
                        self.delegate?.shouldEnableRecordUI(enabled: true, line: "371")
    ...
    @IBAction internal func changeCamera(_ cameraButton: UIButton) {
        cameraButton.isEnabled = false
        
        self.delegate?.shouldEnableRecordUI(enabled: false, line: "390")
        //recordButton.isEnabled = false
        ...
        sessionQueue.async { [unowned self] in
            ...
            DispatchQueue.main.async { [unowned self] in
                ...
                self.delegate?.shouldEnableCameraUI(enabled: true, line: "469")
                //self.cameraButton.isEnabled = true
                
                self.delegate?.shouldEnableRecordUI(enabled: (self.movieFileOutput != nil), line: "472")
    ...
    @IBAction internal func toggleMovieRecording(_ recordButton: UIButton) {
        ...
        self.delegate?.shouldEnableCameraUI(enabled: false, line: "656")
        //cameraButton.isEnabled = false
        
        self.delegate?.shouldEnableRecordUI(enabled: false, line: "659")
        //recordButton.isEnabled = false
    ...
    func capture(_ captureOutput: AVCaptureFileOutput!, didStartRecordingToOutputFileAt fileURL: URL!, fromConnections connections: [Any]!) {
        ...
        DispatchQueue.main.async { [unowned self] in
            ...
            self.delegate?.shouldEnableRecordUI(enabled: true, line: "704")
            //self.recordButton.isEnabled = true
    ...
    func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
        ...
        DispatchQueue.main.async { [unowned self] in
            ...
            self.delegate?.shouldEnableCameraUI(enabled: (self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1), line: "781")
            
            //self.cameraButton.isEnabled = self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1
            
            self.delegate?.shouldEnableRecordUI(enabled: true, line: "785")
            //self.recordButton.isEnabled = true
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if context == &sessionRunningObserveContext {
            ...
            DispatchQueue.main.async { [unowned self] in
                ...
                self.delegate?.shouldEnableCameraUI(enabled: (isSessionRunning && self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1), line: "832")
                //self.cameraButton.isEnabled = isSessionRunning && self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1
                
                self.delegate?.shouldEnableRecordUI(enabled: (isSessionRunning && self.movieFileOutput != nil), line: "835")
